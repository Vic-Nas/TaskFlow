...
for %%F in (*) do (
    if /i not "%%F"=="TaskFlow.exe" if /i not "%%F"=="update.exe" (
        echo Deleting file: %%F
        del /q "%%F"
    )
)

echo Cleanup completed (preserved update.exe).
...
echo - build/dist/TaskFlow folder cleaned (update.exe preserved)
echo - ZIP file created: compiled\win\TaskFlow.zip
echo - Files preserved: TaskFlow.exe, update.exe, data/, tasks/, _internal/
echo ====================================
echo.
pause